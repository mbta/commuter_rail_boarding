defmodule DeparturesCSV do
  ...
  @header_values ~w(
    ...
    Track
    Status)
  @headers Enum.join(@header_values, ",")
  ...
  def to_binary(statuses, unix_now \\ DateTime.to_unix(DateTime.utc_now())) do
    rows =
      for status <- sort_filter(statuses, unix_now) do
        row(status, unix_now)
      end
  ...
  defp should_be_in_output?(%{stop_id: stop_id}, _)
       when stop_id not in ["North Station", "South Station"] do
    false
  end

  defp should_be_in_output?(%{direction_id: direction_id}, _)
       when direction_id != 0 do
    false
  end

  defp should_be_in_output?(
         %{status: :departed, predicted_time: time},
         unix_now
       ) do
  ...
  defp sort_key(status) do
    time =
      if status.predicted_time == :unknown do
        0
      else
